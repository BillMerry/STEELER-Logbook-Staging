...
(async () => {
  ...
  const calculations = await page.evaluate(() => {
    ...
    return {cleaned,original, speed:passageAverageSpeed(a),html,empty:renderPassageCategorySummary([a],{dimension:'all',metrics:[]}),deleted:renderPassageCategorySummary([{...a,deleted:true}],{dimension:'all',metrics:['passages']})};
  });
