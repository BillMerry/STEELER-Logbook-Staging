...
function renderRefillHistory(items) {
  ...
  const footer=(average) => {
    ...
    return `<tr><th scope="row">${average ? "Average / refill" : "Totals"}</th><td></td>${cells(values,average)}<td>${average ? "Available values only; rates weighted" : "Available values only"}</td></tr>`;
  };
  const rows=data.rows.slice().reverse().map(row => `<tr><th scope="row">${escapeHtml(row.date || "Unknown date")}</th><td>${row.full ? "Full" : "Partial"}</td>${cells(row)}<td>${escapeHtml(row.note)}</td></tr>`).join("");
