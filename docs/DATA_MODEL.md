...
The previous per-record sync shape is retained only as historical compatibility data. v1.3.2-rc2 uses one current full-data cloud record instead:
...
Using the cloud copy downloads a local safety backup first, then restores the cloud `steeler-data-backup`. v1.3.2-rc2 also compares the incoming passage plans with the current local copy before restore; if the cloud copy is missing richer local Daily Summary content or has no DPP content where the local passage has DPP content, those local sections are preserved and marked pending so they can be synced back to cloud. The device id key remains local-only and is not restored from the backup.
