...
`sync-worker/` contains an isolated Cloudflare Worker + D1 prototype for the v1.2.0 sync stream. The browser app can now call it manually from Settings for staging-only checks, one-way backup uploads, read-only backup listing, backup JSON download, and guarded cloud-backup restore.
...
It remains deliberately conservative until conflict handling, client push/pull, and multi-device safety testing are ready. The current browser connection can send a complete cloud backup record, list recent cloud backup summaries, download a selected backup JSON file, and manually restore a selected cloud backup after confirmations and a local safety-backup download. It does not automatically pull sync records or merge remote records into local data.
...
- `steeler_sync_config_v1` stores the staging Worker URL and token locally so Settings can test `/v1/status`, send a one-way cloud backup, list recent cloud backup summaries, download a selected backup JSON file, and manually restore a selected cloud backup. The token is not included in full data backups.
