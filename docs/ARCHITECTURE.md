...
`sync-worker/` contains an isolated Cloudflare Worker + D1 prototype for the v1.2.0 sync stream. The browser app can now call it manually from Settings for staging-only checks, one-way backup uploads, read-only backup listing, and backup JSON download.
...
It remains deliberately conservative until conflict handling, client push/pull, and multi-device safety testing are ready. The current browser connection can send a complete cloud backup record, list recent cloud backup summaries, and download a selected backup JSON file, but it does not pull sync records, merge, restore, or overwrite any local data.
...
- `steeler_sync_config_v1` stores the staging Worker URL and token locally so Settings can test `/v1/status`, send a one-way cloud backup, list recent cloud backup summaries, and download a selected backup JSON file. The token is not included in full data backups.
- The manual cloud backup uses `/v1/records/push` with record type `cloud-backup`. The read-only backup list uses `/v1/backups`; selected backup download uses `/v1/backups/{recordId}`. These upload, summarize, or download complete `steeler-data-backup` copies and do not pull or merge remote records into the app.
